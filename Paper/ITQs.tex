...
In the first and second planes a higher value in any of the entropies, $H_{BP}$ and $H_{Val}$, implies an increase in the uniformity of the involved PDF.
The point $(1,1)$ represents the ideal case with uniform histogram and uniform distribution of ordering patterns.
By comparing both planes we can detect fixed points convergences.
...
In both information planes ($H \times C$, see Fig. \ref{fig1} and $H_{Val} \times H_{BP}$, see Fig. \ref{fig2}), stochastic data are clearly localized at different planar positions than deterministic chaotic ones. 
