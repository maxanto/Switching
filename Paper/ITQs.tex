...
The last point in $(H_{Val}, H_{BP})=(0, 0)$ corresponds to a vector of zeros, in this case the histogram of ordering patterns with amplitude contributions is also a zero vector and $H_{BPW}$ can not be calculated.
...
In the figure \ref{fig:InformationPlanes} we show the causality plane $H_{BP}-C_{BP}$.
We can see that not the entire region $0<H_{BP}<1$, $0<C_{BP}<1$ is achievable, in fact, for any PDF the pairs $(H,C)$ of possible values fall between two extreme curves in the plane $H_{BP}-C_{BP}$ \cite{Anteneodo1996}.
...
En Fig. 2 the calculamos HbpXCbp with and without amplitude contributions para los mismos sistemas.
The same sample points are showed to ilustrate the planar positions for different data vectors.
In both information planes ($H_{BP}-C_{BP}$ and $H_{BP}-H_{Val}$ in Fig. \ref{fig:InformationPlanes}), stochastic, chaotic and deterministic data are clearly localized at different planar positions.
