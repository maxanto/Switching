...
In the first plane a higher value in any of the entropies, $H_{BP}$ and $H_{Val}$, implies an increase in the uniformity of the involved PDF.
...
If we order from lowest to highest values the vector generated by ideal white random generator, resulting points are shown by a blue square $(H_{Val}, H_{BP})=(1, 0)$ and a red square $(H_{Val}, H_{BPW})=(1, 0)$.
Blue and red stars shows $(H_{Val}, H_{BP})$ and $(H_{Val}, H_{BPW})$ applied to a ramp signal respectively.
When ramp signal is contaminated with white noise new ordering patterns appear and $H_{BP}$ and $H_{BPW}$ are increased as shown with blue and red triangles.
Finally, we plot the results for logistic map that converges to the fixed point.
