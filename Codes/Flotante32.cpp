...
int main()
{
    ...
    FILE *Results = fopen("Logistico32.dat","w"); //Abre archivo de resultados
    ...
    for (unsigned int iInitialCondition = 0; iInitialCondition < NInitialConditions; iInitialCondition++) // Va sorteando condiciones iniciales
    {
        ...
        {
            Map32[iMap+1] =  4*Map32[iMap]*(1-Map32[iMap]); // Mapa logístico, x[n] = r*x[n-1]*(1-x[n-1]), caótico con r=4. Ni la resta ni la multiplicación por un entero generan fraccionarios
