...
int main()
{
    ...
    float* Map32; //Declare the pointer
        Map32 = (float*) malloc (sizeof(float) * (NIter + 1)); //Creates the array. It has one more postition at first for the length
        Map32[0] = (float)NIter;
    ...
    for (int iBases = 0; iBases <  NBases; iBases++) // Va recorriendo el vector de bases
    {
        ...
        for (int iPrecisions = 0; iPrecisions < NPrecisions; iPrecisions++) //Recorro el vector de precisiones
        {
            ...
            for (unsigned int iInitialCondition = 0; iInitialCondition < NInitialConditions; iInitialCondition++) // Va sorteando condiciones iniciales
            {
                ...
                Map32[1] = (float)InvScale*floor(Scale*InitialConditions[iInitialCondition]); // floorl sirve para long double, como son mapas positivos puedo usar floor en vez de floor
                ...
                {
                    Map32[iMap+1] =  4*InvScale*floor(Scale*Map32[iMap]*(1-Map32[iMap])); // Mapa logístico, x[n] = r*x[n-1]*(1-x[n-1]), caótico con r=4. Ni la resta ni la multiplicación por un entero generan fraccionarios
                } // Acá ya tengo el atractor guardado en el vector Map32

                for (unsigned long int iMap = 1; iMap <= NIter; iMap++) // Va riterando el mapa logístico
                {
                    Map[iMap] =  (double)Map32[iMap]; // Guarda el valor pasado a double
