...
int main()
{
    ...
    FILE *Results = fopen("TentFloat.dat","w"); //Abre archivo de resultados
    ...
    for (unsigned int iInitialCondition = 0; iInitialCondition < NInitialConditions; iInitialCondition++) // Va sorteando condiciones iniciales
    {
        ...
        for (unsigned long int iMap = 1; iMap < NIter; iMap++) // Va riterando el mapa logístico
        {
            if (MapLD[iMap] < invu)
            {
                MapLD[iMap+1] = u*MapLD[iMap];
            }
            else
            {
                MapLD[iMap+1] = kku*(1 - MapLD[iMap]);
            }
...
/*****Debugging section*********
        ...
        printf("ku = %.64Le\n", ku);
        char straux[16];
        sprintf(straux, "TentMap%d.dat", iInitialCondition);
