...
int main()
{
    ...
    FILE *Results = fopen("OddFloat.dat","w"); //Abre archivo de resultados
    ...
    for (unsigned int iInitialCondition = 0; iInitialCondition < NInitialConditions; iInitialCondition++) // Va sorteando condiciones iniciales
    {
        ...
        {
            //Itero logistico
            MapLD[iMap+1] =  4*MapLD[iMap]*(1-MapLD[iMap]);

            //Itero tent
            if (MapLD[iMap+1] < invu)
            {
                MapLD[iMap+1] = u*MapLD[iMap+1];
            }
            else
            {
                MapLD[iMap+1] = kku*(1 - MapLD[iMap+1]);
            }
